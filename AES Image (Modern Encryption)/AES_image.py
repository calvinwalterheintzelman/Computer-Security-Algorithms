...
# image_file: input .ppm image file name #out_file: encrypted .ppm image file name
# key_file: .txt file containing encryption key in ASCII
# Encrypts image_file using CTR mode AES and writes said file to out_file
def ctr_aes_image(iv, image_file='image.ppm', out_file = 'enc_image.ppm', key_file = 'key.txt'):
    k_file = open(key_file, 'r')
    key = k_file.read()  # key as string
    k_file.close()
    ...
    image_v = BitVector(filename=image_file)
    image_header = image_v.read_bits_from_file(8*15)
    o_file = open(out_file, 'wb')
    image_header.write_to_file(o_file)

    # reading from file, encrypting it, then moving on
    curr_iv = iv
    while image_v.more_to_read:
        image_bits = image_v.read_bits_from_file(128)
        block_encrypt = encryption_round(curr_iv, subBytesTable, round_keys)
        output_bits = block_encrypt.__xor__(image_bits)
        output_bits.write_to_file(o_file)
        if int(curr_iv) == 340282366920938463463374607431768211455:  # checks if iv needs to be reset to 0
            curr_iv = BitVector(intVal=0, size=128)
        else:
            curr_iv = BitVector(intVal=int(curr_iv)+1, size=128)
    o_file.close()  # it takes a while to get to here

# initialize parameters that will be sent to function
...
random_number_in_list = x931(v0, dt, 1)
rand_num = random_number_in_list[0]
iv = BitVector(intVal=rand_num, size=128)  # initialize the initialization vector
