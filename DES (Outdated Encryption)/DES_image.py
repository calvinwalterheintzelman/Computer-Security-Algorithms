...
bv_header = bv.read_bits_from_file(8*15)

encrypted_file = open("image_enc.ppm", 'wb')
bv_header.write_to_file(encrypted_file)

file_bit_length = 613023*8 - 15*8
bv_read = bv.read_bits_from_file(file_bit_length)
# bv_read is the bits of the file
im_str = bv_read.get_bitvector_in_ascii()

im_length = len(im_str)

for rounds in round_keys: # all 16 rounds
    i = 0
    new_str = ''
    while i + 8 <= im_length: # the entire image
        re_32 = BitVector(textstring=im_str[i+4:i+8])
        le_32 = BitVector(textstring=im_str[i:i+4])
        ...
        new_le = re_32
        new_str += new_le.get_bitvector_in_ascii()
        new_str += new_re.get_bitvector_in_ascii()
        i += 8
...
i = 0
final_image = ''
while i + 8 <= im_length: # does the final switch!
    re_32 = BitVector(textstring=im_str[i + 4:i + 8])
    le_32 = BitVector(textstring=im_str[i:i + 4])
    final_image += re_32.get_bitvector_in_ascii()
    final_image += le_32.get_bitvector_in_ascii()
    i += 8

bv = BitVector(textstring=final_image)
